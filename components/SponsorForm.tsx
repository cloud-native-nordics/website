        ...
        <div className="mb-4">
          ...
          <div className="space-y-3 mt-2">
            {SPONSORSHIP_TYPES.map((type) => (
              <label key={type.value} className="flex items-start gap-3 p-3 rounded-lg border border-gray-200 dark:border-white/10 cursor-pointer hover:border-nord12/50 transition-colors">
                <input
                  type="radio"
                  name="sponsorship_type"
                  value={type.value}
                  required
                  className="mt-0.5 text-nord12 focus:ring-nord8"
                />
                <span className="text-sm text-nord0 dark:text-gray-300">{type.label}</span>
              </label>
            ))}
          </div>
        </div>
