...
export function Partners() {
  return (
    <section className="py-16 bg-nord6 dark:bg-nord0">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6">
          {CONFERENCES.map((conf) => (
            <a
              ...
              className="flex flex-col items-center gap-3 px-8 py-6 rounded-xl bg-white dark:bg-nord1 border border-gray-200 dark:border-white/10 hover:border-nord12 dark:hover:border-nord12 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 w-full sm:w-auto sm:min-w-[280px]"
            >
              <img src={conf.logo} alt={conf.name} className="h-14 w-auto" />
              <div>
                <div className="font-heading font-bold text-sm text-nord0 dark:text-white">
                  {conf.name}
                </div>
